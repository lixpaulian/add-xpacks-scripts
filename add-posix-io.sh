...
if (( $# < 1 ))
then
    echo "usage: add-posix-io.sh project_root_dir"
    echo "example: bash add-posix-io.sh ~user/Projects/CortexM7"
    exit 1
fi

# project location and definitions
PROJECT_LOCATION=$1
DEST_LOCATION="${PROJECT_LOCATION}/xpacks/posix-io"
DEST_INCLUDE_DIR="${DEST_LOCATION}/include"
DEST_SOURCE_DIR="${DEST_LOCATION}/src"
PROJECT_CONFIG_DIR="${PROJECT_LOCATION}/include"

# package definitions
PACK_LOCATION="$HOME/Projects/xpacks/posix-io.git"
PACK_INCLUDE_DIR="${PACK_LOCATION}/include"
PACK_SOURCE_DIR="${PACK_LOCATION}/src"
PACK_GIT_HOME="https://github.com/lixpaulian/posix-io.git"
